...
class Settings(BaseSettings):
    ...
    EMBED_LOCAL_BATCH_SIZE: int = 16
    EMBED_FAST_BATCH_SIZE: int = 32
    EMBED_FAST_THREADS: int = 0
    EMBED_LOCAL_DEVICE: str = "cpu"
    EMBED_DIM: int = 768
    EMBED_KEEP_ALIVE: str = "5m"
    INGEST_FLUSH_SIZE: int = 32
    AST_MAX_SLIDING_CHUNKS: int = 40
    DOC_MAX_SLIDING_CHUNKS: int = 0
    HTML_MAX_SLIDING_CHUNKS: int = 20
    NOTEBOOK_MAX_CELLS: int = 100
    MAX_CHUNKS_PER_FILE: int = 80
    INGEST_MAX_TOTAL_CHUNKS: int = 3000
    MAX_INDEX_FILE_MB: int = 2
